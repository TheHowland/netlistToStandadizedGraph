...
class ElementFaktory:
    ...
    def make(self, x=0.0, y=0.0, name="", netLine: str = "", size: Vector2D = None, rotation=0, scaling=1.0, default: Element=Element) -> Element:
        """Determines the element type from the first letter of the name R1 -> R type"""
        return self.makeFromType(x,y,name,netLine,size,rotation,scaling,name[0], default)

    def makeFromType(self, x=0.0, y=0.0,name="", netLine="", size: Vector2D = None, rotation=0, scaling=1.0,
                 elmType:str = None, default: Element=Element) -> Element:
        
        element_class = self.type_map.get(elmType, default)
        if element_class:
            return element_class(x, y, name=name, netLine=netLine, size=size, rotation=rotation, scaling=scaling)
        else:
